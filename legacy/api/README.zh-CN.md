...
> **已废弃。** 本服务已迁入 `legacy/api`，不参与默认构建。若需自行启用，见上级 [legacy/README.zh-CN.md](../README.zh-CN.md)。
...
> 把 `legacy/api` 加回 workspace 后，在 **monorepo 根目录**执行 `pnpm install`。见 [legacy/README.zh-CN.md](../README.zh-CN.md)。
...
API 按 `NODE_ENV` 读取 `legacy/api/.env.<环境>`（默认 `.env.development`）。在 `legacy/api/` 下创建 `.env.development`：
...
Docker Compose 下 `DATABASE_URL` / `REDIS_URL` 由 `POSTGRES_*`、`REDIS_PASSWORD` 拼接。见 [legacy/README.zh-CN.md](../README.zh-CN.md)。
...
`assets/` 存放键盘布局 JSON、治具底板与坐标、内置转曲字体。查找与回退逻辑见 [assets/README.zh-CN.md](assets/README.zh-CN.md)。

---

## 部署

生产环境通过 `docker/api.Dockerfile` 多阶段构建并由 Docker Compose 编排，迁移需手动执行。恢复步骤见 [legacy/README.zh-CN.md](../README.zh-CN.md)。
