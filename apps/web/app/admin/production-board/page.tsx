...
function ProductionCard({ item, onAdvance, isPending, isFullscreen }: ProductionCardProps) {
  ...
  const specText = snapshot
    ? (() => {
        ...
        if (layoutName && keysCount) return `${layoutName} · ${keysCount} 键`
        if (layoutName) return layoutName
        if (keysCount) return `${keysCount} 键`
        return null
      })()
    : null
...
export default function ProductionBoardPage() {
  ...
  return (
    ...
    >
      ...
      <PageHeader
        title="生产看板"
        description={isFullscreen ? undefined : "实时查看待生产和生产中的订单任务。"}
        action={
            ...
            <div key={col.status}>
              ...
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border/40 py-14 gap-2">
                  ...
                  <p className="text-xs text-muted-foreground/35">暂无任务</p>
                </div>
              ) : (
